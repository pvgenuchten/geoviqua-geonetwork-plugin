...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="/root/update/parent/gvq:GVQ_Metadata"/>
    </xsl:template>

    <!-- ================================================================= -->

    <xsl:template match="/root/update/parent/gvq:GVQ_Metadata">
        <xsl:copy>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:fileIdentifier"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:language|gmd:characterSet"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:parentIdentifier"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:hierarchyLevel"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:hierarchyLevelName"/>
            ...
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:dataSetURI"/>
        		...
        		/root/child/gvq:GVQ_Metadata/gmd:identificationInfo/*[@gco:isoType='srv:SV_ServiceIdentification']">
	            	<xsl:copy>
	                    ...
	                    <xsl:call-template name="process">
	                        ...
	                        <xsl:with-param name="name" select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:pointOfContact"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:resourceMaintenance"/>
	                    <xsl:copy-of select="gmd:graphicOverview"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:resourceFormat"/>
	                    
	                    <xsl:call-template name="process">
	                        <xsl:with-param name="update" select="$gmd-descriptiveKeywords"/>
	                        <xsl:with-param name="name" select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <!-- FIXME / TO BE DISCUSS following sections are replaced (excepts AggregationInfo preserved). -->
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:resourceSpecificUsage"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:resourceConstraints"/>
	                    <xsl:copy-of select="gmd:aggregationInfo"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:spatialRepresentationType"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:spatialResolution"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:langage"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:characterSet"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:topicCategory"/>
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:environmentDescription"/>
	                    
	                    <xsl:call-template name="process">
	                        <xsl:with-param name="update" select="$gmd-extent"/>
	                        <xsl:with-param name="name" select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:extent"/>
	                        <xsl:with-param name="mode" select="$updateMode"/>
	                        <xsl:with-param name="subLevel" select="true()"/>
	                    </xsl:call-template>
	                    
	                    <!-- FIXME / TO BE DISCUSS following sections are replaced/preserved  -->
	                    <xsl:copy-of select="/root/update/parent/gvq:GVQ_Metadata/gmd:identificationInfo/*/gmd:supplementalInformation"/>
            ...
            <!-- FIXME / TO BE DISCUSS following sections are preserved -->
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:series"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:describes"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:propertyType"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:featureType"/>
            <xsl:copy-of select="/root/child/gvq:GVQ_Metadata/gmd:featureAttribute"/>
