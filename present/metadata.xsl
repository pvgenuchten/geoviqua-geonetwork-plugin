...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform" 
	...
	<!-- create a node-set from the tree fragment generated above -->
	<xsl:variable name="codelistsCombined" select="exslt:node-set($codelistsCopy)"/>
	...
	<xsl:template mode="iso19139.geoviqua"
	...
	-->
	<xsl:template mode="iso19139.geoviqua" priority="199" match="*[@gco:nilReason='missing' and geonet:element and count(*)=1]"/>

	<xsl:template mode="iso19139.geoviqua" priority="199" match="*[geonet:element and count(*)=1 and text()='']"/>

	<!-- ===================================================================== -->
	<!-- these elements should be boxed -->
	<!-- ===================================================================== -->

	<xsl:template mode="iso19139.geoviqua" match="gmd:identificationInfo|gmd:distributionInfo|gmd:descriptiveKeywords|gmd:thesaurusName|gmd:spatialRepresentationInfo|gmd:pointOfContact|gvq:dataQualityInfo|gmd:referenceSystemInfo|gmd:equivalentScale|gmd:projection|gmd:ellipsoid|gmd:extent[name(..)!='gmd:EX_TemporalExtent']|gmd:geographicBox|gmd:EX_TemporalExtent|gmd:MD_Distributor|srv:containsOperations|srv:SV_CoupledResource|gmd:metadataConstraints">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]|
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="gmd:distance" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!--
		...
		-->
	<xsl:template mode="iso19139.geoviqua" match="srv:operatesOn">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:variable name="text">
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="srv:coupledResource/srv:SV_CoupledResource/srv:identifier" priority="200">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="gts:TM_PeriodDuration|gml:duration" priority="100">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
...
<!-- 						<xsl:apply-templates mode="simpleElement" select=".">
	...
	<xsl:template mode="iso19139.geoviqua" match="gco:ScopedName|gco:LocalName">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- GML time interval -->
	<xsl:template mode="iso19139.geoviqua" match="gml:timeInterval">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gml:identifier|gml:axisDirection|gml:descriptionReference">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
  ...
  <!-- Add exception to display downloadable thesaurus differently -->
	...
	<xsl:template mode="iso19139.geoviqua" match="*[*/@codeList and name()!='gmd:country' and name()!='gmd:language' and name()!='gmd:languageCode']">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>

		<xsl:call-template name="iso19139Codelistgeoviqua">
			<xsl:with-param name="schema" select="$schema"/>
			<xsl:with-param name="edit"   select="$edit"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="iso19139Codelistgeoviqua">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		...
		<xsl:apply-templates mode="simpleElement" select=".">
			...
			<xsl:with-param name="edit"   select="$edit"/>
			<xsl:with-param name="text">
				<xsl:apply-templates mode="iso19139GetAttributeTextgeoviqua" select="*/@codeListValue">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:apply-templates>
			</xsl:with-param>
	...
	<xsl:template mode="iso19139GetAttributeTextgeoviqua" match="@*">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
							...
							<xsl:variable name="label">
								<xsl:choose>
									<xsl:when test="/root/gui/codeListDescriptions='on'">
										<xsl:value-of select="gmx:CodeDefinition/gml:description"/>
									</xsl:when>
									<xsl:otherwise>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:dateStamp|gmd:fileIdentifier" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="gmd:PT_Locale/@id"
		priority="2">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:electronicMailAddress" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- gmx:Anchor is a substitute of gco:CharacterString and 
		...
		-->
	<xsl:template mode="iso19139.geoviqua" match="*[gmx:Anchor]" priority="99">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
	...
	<!-- descriptiveKeywords -->
	<!-- ============================================================================= -->
	<xsl:template mode="iso19139.geoviqua" match="gmd:descriptiveKeywords">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
									...
									<xsl:otherwise>
										...
										<xsl:call-template name="translatedString">
											...
											<xsl:call-template name="getLangId">
												...
												<xsl:with-param name="md" select="ancestor-or-self::*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']" />
											</xsl:call-template>
											</xsl:with-param>
										</xsl:call-template>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:keyword[following-sibling::gmd:type/gmd:MD_KeywordTypeCode/@codeListValue='place' and not(following-sibling::gmd:thesaurusName/*/gmd:identifier/*/gmd:code/gmx:Anchor[starts-with(string(),'geonetwork.thesaurus')])]">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:dateTime|gmd:usageDateTime|gmd:plannedAvailableDateTime" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:date[gco:DateTime|gco:Date]|gmd:editionDate|gmd:dateOfNextUpdate" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gml:*[gml:beginPosition|gml:endPosition]|gml:TimeInstant[gml:timePosition]" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:for-each select="*">
			<xsl:choose>
	...
	<xsl:template mode="iso19139.geoviqua" match="*[geonet:info/isTemplate='s']" priority="3">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="@gco:isoType"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gvq:GVQ_Metadata|*[@gco:isoType='gvq:GVQ_Metadata']">
		...
		<xsl:choose>
			...
			<xsl:when test="$currTab='metadata'">
				<xsl:call-template name="iso19139GeoviquaMetadata">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:call-template>
			</xsl:when>
			...
			<!-- ISOMinimum tab -->
			<xsl:when test="$currTab='geoviquaMinimum'">
				<xsl:call-template name="geoviqua">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
			...
			<!-- ISOCore tab -->
			<xsl:when test="$currTab='geoviquaCore'">
				<xsl:call-template name="geoviqua">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
					<xsl:with-param name="dataset" select="$dataset"/>
	...
	<xsl:template name="geoviqua">
		...
		<!-- dataset or resource info in its own box -->
 		...
 		<xsl:for-each select="gmd:identificationInfo/gvq:GVQ_DataIdentification|
						...
						gmd:identificationInfo/*[@gco:isoType='gvq:GVQ_DataIdentification']|
...
<!-- 		<xsl:apply-templates mode="elementEP" select="gmd:metadataExtensionInfo|geonet:child[string(@name)='metadataExtensionInfo']">
	...
	<xsl:template name="iso19139GeoviquaMetadata">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
...
<!-- 	<xsl:template name="iso19139Simple">
	...
	<xsl:template mode="iso19139.geoviqua" match="//gmd:language[gco:CharacterString]" priority="20">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
...
<!-- 	<xsl:template name="iso19139Simple2">
			...
			<xsl:if test="count(gmd:MD_DigitalTransferOptions/gmd:onLine/updated19115:CI_OnlineResource/gmd:protocol/gco:CharacterString[contains(string(.),'download')])>1 and
				...
				<xsl:call-template name="complexElementGui">
					...
					<xsl:with-param name="content">
						<tr>
							<td  align="center">
								<button class="content" onclick="javascript:runFileDownloadSummary('{/root/*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/uuid}', '{/root/gui/strings/downloadSummary}')" type="button">
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:contact|gmd:pointOfContact|gmd:citedResponsibleParty">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
...
<!-- 	<xsl:template name="contactTemplate">
		...
		<xsl:variable name="langId">
			<xsl:call-template name="getLangId">
				...
				<xsl:with-param name="md"
					select="ancestor-or-self::*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']" />
			</xsl:call-template>
		</xsl:variable>
	...
	<xsl:template mode="iso19139.geoviqua" match="updated19115:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-map') and gmd:name]|updated19115:CI_OnlineResource[gmd:protocol/gco:CharacterString = 'OGC:WMS' and string(gmd:name/gco:CharacterString)]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="/root/*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info[dynamic='true'] and string($name)!='' and string($linkage)!=''">
				...
				<xsl:apply-templates mode="simpleElement" select=".">
					...
					<xsl:with-param name="text">
						<a href="{/root/gui/locService}/google.kml?uuid={/root/*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/uuid}&amp;layers={$name}" title="{/root/strings/interactiveMap}">
							<xsl:choose>
								<xsl:when test="string($description)!=''">
									<xsl:value-of select="$description"/>
								</xsl:when>
			...
			<xsl:when test="string(ancestor::*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/dynamic)='true' and string($linkage)!='' and not(string($name))">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema"  select="$schema"/>
					<xsl:with-param name="title"  select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139.geoviqua" match="updated19115:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-capabilities')]|updated19115:CI_OnlineResource[gmd:protocol/gco:CharacterString = 'OGC:WMS' and not(string(gmd:name/gco:CharacterString))]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(/root/*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/dynamic)='true' and string($linkage)!=''">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema"  select="$schema"/>
					<xsl:with-param name="title"  select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139.geoviqua" match="updated19115:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'ESRI:AIMS-') and contains(gmd:protocol/gco:CharacterString,'-get-image') and gmd:name]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(/root/*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/dynamic)='true' and string($linkage)!='' and string($name)!=''">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema"  select="$schema"/>
					<xsl:with-param name="title"  select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139.geoviqua" match="updated19115:CI_OnlineResource[matches(gmd:protocol/gco:CharacterString,'^WWW:DOWNLOAD-.*-http--download.*') and gmd:name]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(ancestor::*[name(.)='gvq:GVQ_Metadata' or @gco:isoType='gvq:GVQ_Metadata']/geonet:info/download)='true' and string($linkage)!='' and not(contains($linkage,$download_check))">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema"  select="$schema"/>
	...
	<xsl:template mode="iso19139.geoviqua" match="gmd:protocol" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		...
		<metadata>
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<!-- now brief elements for geoviqua specific elements -->
	...
	<xsl:template name="iso19139.geoviqua-brief">
		...
		<xsl:apply-templates mode="briefster" select="gmd:identificationInfo/gvq:GVQ_DataIdentification|gmd:identificationInfo/*[@gco:isoType='gvq:GVQ_DataIdentification']|gmd:identificationInfo/srv:SV_ServiceIdentification">
			<xsl:with-param name="id" select="$id"/>
			<xsl:with-param name="langId" select="$langId"/>
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="gmd:country[gco:CharacterString]" priority="1">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
    ...
    -->
    <xsl:template mode="iso19139.geoviqua" match="gmd:parentIdentifier|gmd:code[name(../..)='gmd:aggregateDataSetIdentifier']"
        priority="2">
        <xsl:param name="schema" />
        <xsl:param name="edit" />
	...
	<!-- Display extra thumbnails (not managed by GeoNetwork).
		 ...
		 simple elements.
	-->
	<xsl:template mode="iso19139.geoviqua" match="gmd:graphicOverview" priority="2">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
	...
	-->
	<xsl:template mode="iso19139.geoviqua" match="gmd:locale|geonet:child[string(@name)='locale']" priority="1">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
		<xsl:choose>
			<xsl:when test="$edit = true()">
